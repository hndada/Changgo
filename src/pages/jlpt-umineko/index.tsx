...
type Props = {
  ...
  page: number,
  'pageCount': number,
  'defaultQ': string|null, // default query
    ...
    <form>
      ...
      {/* <label>
        ...
        <select name="level" defaultValue={defaultLevel ?? -1}>
          ...
          <option value="0">5급</option>
          <option value="1">4급</option>
          <option value="2">3급</option>
          <option value="3">2급</option>
          <option value="4">1급</option>
        </select>
      </label> */}
...
export const getServerSideProps:GetServerSideProps<Props> = async context => {
  ...
  return {
    props: {
      ...await fetch(`${origin}/api/jlpt-search${search}`).then(res => res.json()),
